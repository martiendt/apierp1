...
export async function createCollection(db: IDatabaseAdapter) {
  try {
    ...
    await db.updateSchema(collection, {
      bsonType: "object",
      required: ["name", "number", "type", "category", "increasing_in"],
      properties: {
        name: {
          bsonType: "string",
          description: "The name for the coa",
        },
      },
    });
    ...
    console.info(`[schema] ${collection} - create unique attribute "number"`);
    await helper.createUnique(collection, {
      number: -1,
    });
  } catch (error) {
