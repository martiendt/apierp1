import { RetrieveAllCustomerGroupRepository } from "../model/repository/retrieve-all.repository.js";
...
export class RetrieveAllCustomerGroupUseCase {
  ...
  public async handle(query: QueryInterface, options: RetrieveAllOptionsInterface) {
    try {
      ...
      const response = await new RetrieveAllCustomerGroupRepository(this.db).handle(query, options);
