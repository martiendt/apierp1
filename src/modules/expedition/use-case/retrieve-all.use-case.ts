import { RetrieveAllExpeditionRepository } from "../model/repository/retrieve-all.repository.js";
import DatabaseConnection, { QueryInterface, RetrieveAllOptionsInterface } from "@src/database/connection.js";
import { VerifyTokenUseCase } from "@src/modules/user/use-case/verify-token.use-case.js";

export class RetrieveAllExpeditionUseCase {
  ...
  public async handle(query: QueryInterface, options: RetrieveAllOptionsInterface) {
    try {
      ...
      const response = await new RetrieveAllExpeditionRepository(this.db).handle(query, options);
