import { RetrieveAllSupplierGroupRepository } from "../model/repository/retrieve-all.repository.js";
...
export class RetrieveAllSupplierGroupUseCase {
  ...
  public async handle(query: QueryInterface, options: RetrieveAllOptionsInterface) {
    try {
      ...
      const response = await new RetrieveAllSupplierGroupRepository(this.db).handle(query, options);
