import { RetrieveAllAllocationRepository } from "../model/repository/retrieve-all.repository.js";
...
export class RetrieveAllAllocationUseCase {
  ...
  public async handle(query: QueryInterface, options: RetrieveAllOptionsInterface) {
    try {
      ...
      const response = await new RetrieveAllAllocationRepository(this.db).handle(query, options);
