...
import { UpdateAllocationGroupRepository } from "../model/repository/update.repository.js";
...
export class UpdateAllocationGroupUseCase {
  ...
  public async handle(id: string, document: DocumentInterface, options: UpdateOptionsInterface) {
    try {
      ...
      const allocationGroupRepository = new UpdateAllocationGroupRepository(this.db);
      await allocationGroupRepository.handle(id, objClean(allocationGroupEntity), options);
