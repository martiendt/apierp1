import { RetrieveAllAllocationGroupRepository } from "../model/repository/retrieve-all.repository.js";
...
export class RetrieveAllAllocationGroupUseCase {
  ...
  public async handle(query: QueryInterface, options: RetrieveAllOptionsInterface) {
    try {
      ...
      const response = await new RetrieveAllAllocationGroupRepository(this.db).handle(query, options);
